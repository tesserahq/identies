...
class DeleteApiKeyCommand:
    ...
    def execute(self, api_key_id: UUID, user_id: UUID) -> bool:
        """
        ...
        try:
            ...
            self._publish_api_key_deleted_event(api_key, user_id)
    ...
    def _publish_api_key_deleted_event(self, api_key: ApiKey, user_id: UUID) -> None:
        """
        Publish an API key deleted event.

        Args:
            api_key: The deleted API key
            user_id: The ID of the user who deleted the API key
        """
        event = build_api_key_deleted_event(api_key, user_id)
