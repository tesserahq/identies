"""Tests for ExternalAccountRepository."""
...
def test_create_link_token_expires_at_approx_10_min(db, faker):
    ...
    repository = ExternalAccountRepository(db)
    token_value, expires_at = repository.create_link_token(
        platform="telegram",
...
def test_consume_link_token_creates_external_account(db, setup_user, setup_link_token):
    ...
    consumed = repository.consume_link_token(link_token.token)
    assert consumed is not None
    assert consumed.used_at is not None
    account = repository.create_external_account(
...
def test_consume_link_token_twice_second_fails(db, setup_link_token):
    ...
    first = repository.consume_link_token(token)
    assert first is not None
    second = repository.consume_link_token(token)
...
def test_consume_expired_token_returns_none(db, setup_expired_link_token):
    ...
    repository = ExternalAccountRepository(db)
    consumed = repository.consume_link_token(setup_expired_link_token.token)
...
def test_consume_used_token_returns_none(db, setup_used_link_token):
    ...
    repository = ExternalAccountRepository(db)
    consumed = repository.consume_link_token(setup_used_link_token.token)
    assert consumed is None
...
def test_get_external_account_scoped_to_user(
    ...
    found = repository.get_external_account(account.id, setup_user.id)
    assert found is not None
    assert found.id == account.id
    found_other = repository.get_external_account(account.id, setup_another_user.id)
...
def test_get_external_accounts_query_returns_only_user_accounts(
    ...
    repository = ExternalAccountRepository(db)
    account = setup_external_account
    query = repository.get_external_accounts_query(setup_user.id)
    items = query.all()
    assert len(items) >= 1
    assert any(a.id == account.id for a in items)
    query_other = repository.get_external_accounts_query(setup_another_user.id)
...
def test_delete_external_account_owner_succeeds(db, setup_user, setup_external_account):
    ...
    repository = ExternalAccountRepository(db)
    account = setup_external_account
    ok = repository.delete_external_account(account.id, setup_user.id)
    assert ok is True
    found = repository.get_external_account(account.id, setup_user.id)
...
def test_delete_external_account_non_owner_fails(
    ...
    repository = ExternalAccountRepository(db)
    account = setup_external_account
    ok = repository.delete_external_account(account.id, setup_another_user.id)
    assert ok is False
    found = repository.get_external_account(account.id, setup_user.id)
    assert found is not None
