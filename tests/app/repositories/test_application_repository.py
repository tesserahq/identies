...
def test_create_application(db: Session, sample_application_data):
    ...
    application = ApplicationRepository(db).create_application(application_create)
...
def test_get_application(db: Session, sample_application):
    ...
    retrieved = ApplicationRepository(db).get_application(sample_application.id)
...
def test_get_applications(db: Session, sample_application):
    ...
    applications = ApplicationRepository(db).get_applications()
...
def test_get_applications_with_pagination(db: Session, sample_application):
    ...
    applications = ApplicationRepository(db).get_applications(skip=0, limit=1)
    assert len(applications) == 1

    applications = ApplicationRepository(db).get_applications(skip=1, limit=1)
    assert len(applications) == 1
...
def test_update_application(db: Session, sample_application):
    ...
    updated = ApplicationRepository(db).update_application(
...
def test_delete_application(db: Session, sample_application):
    ...
    success = repository.delete_application(sample_application.id)

    assert success is True
    assert repository.get_application(sample_application.id) is None
...
def test_application_not_found_cases(db: Session):
    ...
    assert repository.get_application(non_existent_id) is None

    update_data = {"description": "Updated"}
    application_update = ApplicationUpdate(**update_data)
    assert repository.update_application(non_existent_id, application_update) is None

    assert repository.delete_application(non_existent_id) is False
...
def test_search_applications(db: Session, sample_application):
    ...
    results = ApplicationRepository(db).search(filters)
...
def test_create_applications_batch(db: Session, sample_application_data):
    ...
    applications = repository.create_applications_batch([app1, app2])
...
def test_get_applications_query_with_search(db: Session, sample_application):
    ...
    query = repository.get_applications_query(q=sample_application.name)
    results = query.all()
    assert any(a.id == sample_application.id for a in results)

    query = repository.get_applications_query(q="nonexistent")
    results = query.all()
